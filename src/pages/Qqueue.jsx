...
function Queue() {
  ...
  const fetchQueue = async (idToLoad) => {
    ...
    try {
      ...
      const data_json = !voucarregar_id ? json.data.data : json.data;
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4 text-center">Fila de Consultas</h1>
      <div className="mb-4">
        <TextInput
          multiline
          numberOfLines={5}
      ...
      <h2 className="text-lg font-semibold mb-2">Itensgrr na Fila</h2>
      {queueItems.length > 0 ? (
        <ul className="list-disc pl-5">
          {queueItems.map(item => (
            <li key={item.id}>
              <Link to={`/qqueue/${item.id}`} className="text-blue-600">BIN: {item.card_bin} - Status: {item.status}</Link>
            </li>
          ))}
        </ul>
      ) : ( 
        <>
          {queueItems.items.map(item => (<QueueItem queue_data={item} />))}
          /*<QueueItem
